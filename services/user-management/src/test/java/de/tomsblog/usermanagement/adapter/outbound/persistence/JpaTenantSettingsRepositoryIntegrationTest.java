...
class JpaTenantSettingsRepositoryIntegrationTest {
    ...
    void saveUpdatesExisting() {
        var tenantId = TenantId.generate();
        var settings = TenantSettings.reconstitute(tenantId, LoginMode.BOTH, false, Set.of());
        repository.save(settings);

        var updated = TenantSettings.reconstitute(tenantId, LoginMode.INTERNAL, true, Set.of("corp.com"));
    ...
    void emailDomainsPersistedCorrectly() {
        var tenantId = TenantId.generate();
        var settings = TenantSettings.reconstitute(tenantId, LoginMode.BOTH, false, Set.of("a.com", "b.org", "c.net"));
